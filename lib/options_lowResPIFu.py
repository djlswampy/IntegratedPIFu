...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # start option check
        parser.add_argument('--useDOS', default=False, help='depth oriented sampling') # low-pifu는 사용하지 않음(이 옵션을 False로 사용하면 배치로 넘어가지 않고 메모리가 계속해서 올라감)
        parser.add_argument('--use_mask_for_rendering_high_res',default=False) # low-pifu는 사용하지 않음
        parser.add_argument('--use_mask_for_rendering_low_res',default=False) # Not used in the paper.


        parser.add_argument('--update_low_res_pifu', default=False) # low-pifu는 사용하지 않음
        ...
        # high res component
        parser.add_argument('--use_High_Res_Component', default=False) # low-pifu는 사용하지 않음
